...
## fix-3aug (rig verdicts): cover-crop + visible speckles
The continuum rig convicted two renders-side issues. (1) LETTERBOX DEAD: every scale path (static branch + kling scale_pad) now cover-crops (force_original_aspect_ratio=increase + crop) instead of padding -- non-16:9 sources fill the frame by center-crop, the old Mission Control fix made automatic and universal. (2) SPECKLES VISIBLE: the field generates at half-res with 2x2 speck bodies (nearest upscale + gblur 0.7), surviving blend into readable soft glow-specks (measured: ~1,000 visible px on a dark 640x360 frame at strength 0.35; screen-blend remains invisible over pure whites by physics, correct behavior). Tail-vs-floor amplitude asymmetry reviewed and ruled BY DESIGN (continuation grammar is deliberately gentler); dial exists if real content ever convicts it. REAL FAL RATES banked from the rig dashboard: stills $0.04, kling v2.5-turbo-pro $0.07/s (5s = $0.35) -- intake BOM constants to re-base next kit rev; estimates until then are ~40% conservative.

## tail-speckle parity (3 Aug, Peter-ruled continuum decision)
The atmosphere doctrine, final: FLOOR beats specked (live) + KLING TAILS specked (this patch: same overlay in _fit_to_duration's tail branch, speckle layer fades in from black over 0.8s -- black is a no-op under screen blend, so the fade IS the opacity ramp and the seam stays clean by construction) + RAW KLING clean BY DESIGN (the model makes its own air; compositing static drift over generated motion reads as a dirty lens) + strength one channel dial (sacred-dawn 0.35, signed off). Verified: native 0 specks, tail ramps 0 -> ~1,000 over 0.8s, baseline seam PSNR 42.
