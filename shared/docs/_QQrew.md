...
*The comprehensive, durable craft + config + strategy brief for @Q-Qrew. Single-underscore = channel doctrine (load per channel); the double-underscore canonical reference is the system. Where this doc and the Final-Hours-derived craft in `ante-machinam.md` Part IV disagree, **THIS DOC WINS for this channel** — see §12 the break-list.*
...
- **Character determinism:** Driver lock in `base_canon` (auto-merges every beat) + `people_directive` in the rulebook. Specificity kills drift.
- **Category (metadata):** **Education (ID 27)** — NOT Entertainment (24). Education routes to the right suggested-video pool AND a higher-RPM ad bucket. (channel.json category_id should be "27" for the auto-upload step; crew-wip currently has "24" inherited from success-coach — fix.)
...
The canonical craft (`ante-machinam.md` Part IV) is really the Final-Hours/Sacred-Dawn brief — Final Hours was built first and infected the "channel-agnostic" docs. This channel deliberately breaks:
