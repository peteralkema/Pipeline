...
- **Register:** reverent, warm, dignified, story-first. The voice of a trusted storyteller telling a true story straight — not a mystery-monger, not a hype-man. Present-tense cinematic witness. Never "BANNED / FORBIDDEN / what they're hiding / NOT human."
...
**The look is a primary brand-separator — now reinforced by voice.** Earlier this channel was set to share Elliot with Sacred Dawn, which put the whole separation load on the look. With **Ren** selected here and Elliot on Sacred Dawn, the two channels are now distinct in *both* voice and look — a stronger, cleaner separation. The look still does heavy lifting and remains load-bearing, not aesthetic. Sacred Dawn is ash, storm-grey, weathered gloom; **this channel is the inverse: vivid, jewel-toned, luminous.** Think *Joseph and the Amazing Technicolor Dreamcoat*, *The Prince of Egypt*'s colour drama, Renaissance oil-painting richness. The target tier is the **"Amazon Prime" of the Bible category** (§1) — prestige streaming-cinematic, at ~$20/video.

- **The cartoon trap (the one caution):** "vivid colourful" must be **photoreal-painterly vividness, not flat cartoon saturation** — or it collapses into the animation swarm we're differentiating from. The colour comes from *materials, ornament, and golden-hour light rendered photoreally* (real fabric, real gold, real lapis), deep and luminous — never bright-and-flat. This keeps us out of both Sacred Dawn's gloom and the cartoon pile.
...
- **`style_suffix` (draft):**
  ...
  >
  > Deliberately absent: "ash," "storm-grey," "weathered," "muted" (Sacred Dawn's words). This is the inverse palette.
...
- **Motion direction — *per-channel guidance now; per-clip discretion on Mission Control*.** Current design: motion is a **channel-level guidance setting** (the channel's overall pan/zoom character), with **per-clip override discretion available on the Mission Control page**. Carry over Sacred Dawn's dramatic pan/zoom interplay, but run it **livelier** — human drama wants more energy than cosmic reverence. Default to a slow, intimate push-in character with the dramatic-reveal pulls Sacred Dawn developed; adjust per clip on Mission Control as the beat needs.
...
- **Thumbnail:** the figure in the cinematic ancient world + the tension the title doesn't state. Photoreal face-adjacent or faceless-with-scale, period-accurate, a single legible emotional beat. Text as a layer (models can't render legible type). Squint-test at ~120px.
- **Lean into female leads** (the over-indexing pattern) in both the slate and the thumbnails.
- **Avoid:** the apocrypha/forbidden register entirely (that's Sacred Dawn / Audience B — "BANNED," "the book they hid," "NOT human"); the sepia "biblical-mystery" wash; cartoon/animation aesthetics (the swarm we're differentiating from); stock reaction-faces and pointing fingers.
...
*Maintained by Peter + Claude. Update when the name lands, when the voice is chosen, when the first-video retention data is in, and when the competitive cohort shifts. This is the channel's creed: the canonical Bible, told faithfully, rendered photoreal, for the faithful audience Sacred Dawn doesn't serve.*
