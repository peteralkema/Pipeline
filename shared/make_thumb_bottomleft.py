...
def main():
    ...
    ap.add_argument("--title2", default="", help="optional second title line, stacked under --title for a much larger headline")
    ...
    title2 = (a.title2.upper() if upper else a.title2) if a.title2 else ""
    ...
    # title lines fill more of the frame for a dominant flagship headline
    title_w_cap = int(W * t.get("title_area_pct_large", 0.66))
    if title2:
        # two stacked lines: each line gets ~26% of height, so the block is large
        line_h_cap = int(H * t.get("title_line_max_height_pct", 0.26))
        tf, tused, tw, th = fit(draw, title, font_path, fb,
                                title_w_cap, line_h_cap,
                                int(t.get("title_start_size", 150)), stroke)
        tf2, tused2, tw2, th2 = fit(draw, title2, font_path, fb,
                                    title_w_cap, line_h_cap,
                                    int(t.get("title_start_size", 150)), stroke)
        # match the two lines to the smaller of the two sizes so they read as one block
        # (re-fit the larger line down to the smaller font for visual consistency)
    else:
        tf, tused, tw, th = fit(draw, title, font_path, fb,
                                title_w_cap,
    ...
    gap = int(th * 0.14)
    line_gap = int(th * 0.10)
    if title2:
        block_h = th + line_gap + th2 + gap + sh_
    else:
        block_h = th + gap + sh_
    x = mx
    y_title = H - my - block_h
    if title2:
        y_title2 = y_title + th + line_gap
        y_sub = y_title2 + th2 + gap
    else:
    ...
    if title2:
        render(title2, tf2, (x, y_title2), title_color)
